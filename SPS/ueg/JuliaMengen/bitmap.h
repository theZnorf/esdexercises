//       $Id: bitmap.h 29419 2015-10-13 06:46:29Z p20068 $
//      $URL: https://svn01.fh-hagenberg.at/bin/cepheiden/vocational/teaching/ESD/SPS3/2015-WS/Ablauf/src/Fractals/src/bitmap.h $
// $Revision: 29419 $
...
typedef std::uint8_t  byte_t;  static_assert (sizeof (pfc::byte_t)  == 1, "PFC: The size of 'pfc::byte_t' must be 1." );
typedef std::uint32_t dword_t; static_assert (sizeof (pfc::dword_t) == 4, "PFC: The size of 'pfc::dword_t' must be 4.");
typedef std::int32_t  long_t;  static_assert (sizeof (pfc::long_t)  == 4, "PFC: The size of 'pfc::long_t' must be 4." );
typedef std::uint16_t word_t;  static_assert (sizeof (pfc::word_t)  == 2, "PFC: The size of 'pfc::word_t' must be 2." );

typedef std::false_type is_integral_no;
typedef std::true_type  is_integral_yes;
typedef std::false_type is_unsigned_no;
typedef std::true_type  is_unsigned_yes;
...
template <typename value_t> inline value_t *
   mem_copy_ptr (value_t * const p_dst, value_t const * const p_src, std::size_t const size);
template <typename value_t> inline value_t &
   mem_reset (value_t & dst);
template <typename value_t> inline value_t &
   mem_set (value_t & dst, int const c = 0);
template <typename value_t> inline value_t *
   mem_set_ptr (value_t * const p_dst, int const c, std::size_t const size);
inline void
...
template <typename value_t> inline std::istream &
   read_ptr (std::istream & in, value_t * const ptr, std::size_t const count = 1);
template <typename value_t> inline std::istream &
   read (std::istream & in, value_t & val);
template <typename value_t> inline std::ostream &
   write_ptr (std::ostream & out, value_t const * const ptr, std::size_t const count = 1);
template <typename value_t> inline std::ostream &
...
class bitmap {
   ...
   public:
      ...
      bitmap (std::size_t const width, std::size_t const height, pfc::byte_t * p_image = nullptr) {
      ...
      void create (std::size_t const width, std::size_t const height, pfc::byte_t * p_image = nullptr) {
//       assert (((width * 3) % 4 == 0) && "a scanline must end on a 32-bit boundary");
      ...
      std::size_t get_num_pixels () const {
   ...
   private:
      template <typename value_t> static std::unique_ptr <pfc::byte_t []> allocate (std::size_t const size) {
      ...
      static bool is_valid (info_header_t const & hdr) {
         return (hdr.size              == sizeof (info_header_t)) &&
                ...
                (hdr.compression       == 0) &&
                (hdr.size_image        == size_image (hdr.width, hdr.height)) &&
                (hdr.clr_used          == 0) &&
                (hdr.clr_important     == 0) &&
                (size_line (hdr.width) == size_scanline (hdr.width));
      }

      static std::size_t size_image (std::size_t const width, std::size_t const height) {
         return size_scanline (width) * height;
      }

      static std::size_t size_line (std::size_t const width) {
         return width * c_bpp;
      }

      static std::size_t size_scanline (std::size_t const width) {   // a scanline must end on a 32-bit boundary
         return pfc::ceil_div (size_line (width), std::size_t (4)) << 2;
      }
...
template <typename value_t> inline value_t * mem_copy_ptr (value_t * const p_dst, value_t const * const p_src, std::size_t const size) {
...
template <typename value_t> inline value_t * mem_set_ptr (value_t * const p_dst, int const c, std::size_t const size) {
...
template <typename value_t> inline std::istream & read_ptr (std::istream & in, value_t * const ptr, std::size_t const count) {
...
template <typename value_t> inline std::ostream & write_ptr (std::ostream & out, value_t const * const ptr, std::size_t const count) {
