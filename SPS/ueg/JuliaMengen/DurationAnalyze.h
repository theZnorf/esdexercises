...
namespace julia
{
    ...
    public:
        ...
        using AdditionalTagType = size_t;
    ...
    public:
        template <typename T, typename TF>
        void RunCalculation(CalculationPointer<T, TF> pCalc, TagType const & tag, AdditionalTagType const & additionalTag, bool printDuration = false, bool saveFile = false, std::string const & filePrefix = "")
        {
            TF imagBegin = -1.0;
            TF imagEnd = 1.0;
            TF imagStep = 0.5;
            ...
            for (auto imag = imagBegin; imag <= imagEnd; imag += imagStep)
            {
                for (auto real = realBegin; real <= realEnd; real += realStep)
                {
                    ...
                    durations.push_back(pfc::timed_run(1, std::mem_fn(&JuliaFractalCalculation<T, TF>::Calc), pCalc.get(), c));
